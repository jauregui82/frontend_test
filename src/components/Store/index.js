...
export const Store = (props) => {
    ...
    const selectedCell = async (id) =>{
        const dataFiltred = dataCounter.find((item)=>{return id=== item.id});
        setDataCounterSelected(dataCounterSelected => [...dataCounterSelected, dataFiltred]);        
    }
    const verifyFindCounter = (id) =>{
        const dataSelected = dataCounterSelected.find((item)=>{return item.id===id});
